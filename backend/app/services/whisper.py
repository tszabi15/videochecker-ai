...
Transcribes audio via the Groq Cloud API (whisper-large-v3-turbo) with
retry logic and dynamic 429 backoff handling.
"""
...
def transcribe_with_groq(audio_path: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        raise ImportError("The 'groq' package is required for Groq transcription.")
    ...
    for attempt in range(max_retries):
        try:
            ...
            err_str = str(e)
            logger.warning(
                "Groq transcription attempt %d/%d failed: %s", attempt + 1, max_retries, err_str,
            )
            if attempt == max_retries - 1:
                logger.error(
                    "Groq transcription CRITICAL — all retries exhausted for %s: %s",
                    audio_path, err_str,
                )
                raise RuntimeError(
                    f"Groq transcription engine unavailable after {max_retries} retries: {err_str}"
                ) from e
            ...
            if "429" in err_str or "rate_limit" in err_str.lower() or "quota" in err_str.lower():
                ...
                if match:
                    ...
                    logger.info("Groq rate limit 429. Sleeping %.2fs...", sleep_time)
                else:
...
class WhisperService:
    """Wrapper maintaining compatibility with service layer callers."""

    def transcribe(self, audio_wav_path: str) -> Dict[str, Any]:
        """Transcribes audio and returns {'segments': [...]}."""
        try:
            segments = transcribe_with_groq(audio_wav_path)
            return {"segments": segments}
        except Exception as e:
            logger.error("WhisperService transcription error: %s", e)
            raise
